...
export class StudioServiceHttp extends StudioService {
  ...
  getStudio(login: string): Observable<Studio> {
    return this.http.get(`${ConfigurationProvider.apiUrl}studio/getFull/${login}`).map(x => Studio.ToStudio(x.json()));
  }

  updateStudio(studio: Studio): Observable<BaseResponse>{
    return this.http.post(`${ConfigurationProvider.apiUrl}studio/edit`, studio).map(x => BaseResponse.ToBaseResponse(x.json()));
  }
